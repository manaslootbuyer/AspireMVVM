...
namespace MvvmAspire.Controls
{
    public class RoundedButtonRenderer : ButtonControlRenderer
    {
                ...
                if (Control != null)
                {
                    ...
                    Control.TouchDown += (s, evt) =>
                    {
                        if (Control != null)
                        {
                            ...
                            {
                                await BaseElement.ScaleTo(0.9, 100);
                            });
                    ...
                    Control.TouchCancel += (s, evt) =>
                    {
                        if (Control != null)
                        {
                            DispatchQueue.MainQueue.DispatchAsync(async () =>
                           {
                               ...
                               await BaseElement.ScaleTo(1, 100);
                           });
                            Control.Selected = false;
                        }
                    };

                    Control.TouchUpOutside +=  (s, evt) =>
                    {
                        if (Control != null)
                        {
                            DispatchQueue.MainQueue.DispatchAsync(async () =>
        ...
        void SetShadow(RoundedButton element)
        {
            ...
            {
                Control.Layer.RenderButtonShadow(element);
            }
        }
